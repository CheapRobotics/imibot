...
class HMC5883L:
    ...
    *Settings
    ...
    def get_field_x(self):
        _x = self.bus.read_byte_data(self.DEV_ADDR, self.REG_X_MSB) << 8 | self.bus.read_byte_data(self.DEV_ADDR,
                                                                                                   self.REG_X_LSB)
        _x = self._two_comp_to_dec(_x)
        return _x * self.meas_res

    def get_field_y(self):
        _y = self.bus.read_byte_data(self.DEV_ADDR, self.REG_Y_MSB) << 8 | self.bus.read_byte_data(self.DEV_ADDR,
                                                                                                   self.REG_Y_LSB)
        _y = self._two_comp_to_dec(_y)
        return _y * self.meas_res

    def get_field_z(self):
        _z = self.bus.read_byte_data(self.DEV_ADDR, self.REG_Z_MSB) << 8 | self.bus.read_byte_data(self.DEV_ADDR,
                                                                                                   self.REG_Z_LSB)
